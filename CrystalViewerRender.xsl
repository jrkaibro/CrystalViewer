...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="GxControl">
    <xsl:choose>
      <xsl:when test="@type = 'CrystalViewer'">
        <xsl:call-template name="RenderCrystalViewer"/>
      </xsl:when>
    </xsl:choose>
  </xsl:template>

  <!-- CrystalViewer design render -->
  <!-- ///////////////////  Implement your render here  ///////////////////-->
  <xsl:template name="RenderCrystalViewer">
     ...
     <img style='width:370px;height:200px;'>
        <xsl:attribute name="src">
          <xsl:value-of select='gxca:GetMyPath()'/>
		   <xsl:text>\crystalreport.png</xsl:text>
        </xsl:attribute>
      </img>      
    </span>
  </xsl:template>
